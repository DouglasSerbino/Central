...
class Lista_pedidos_m extends CI_Model {
  ...
  /**
	 ...
	 *@return nada.
	*/
	function listar($Id_Proceso, $Eres_Grupo = false)
	{
		...
		if(false == $Eres_Grupo)
		{
			$Consulta_Where = 'and id_grupo = "'.$this->session->userdata('id_grupo').'"';
		}/*
		else
		{
			//$Consulta_Where = 'and fecha_entrada <= "2012-05-14"';
		}*/
		
		$Consulta = '
			select id_pedido, fecha_entrada, fecha_entrega, fecha_reale
		...
		if(0 < $Resultado->num_rows())
		{
			...
			{
				$Pedidos['info'][$Fila['id_pedido']] = $Fila;
				$Id_Pedidos[] = $Fila['id_pedido'];
			}
			
			
			$Consulta = '
				select id_pedido_adjunto_reproceso, id_pedido
				from pedido_adjunto_reproceso
				where id_pedido in ('.implode(',', $Id_Pedidos).')
				order by id_pedido_adjunto_reproceso desc
			';
			$Resultado = $this->db->query($Consulta);
			foreach($Resultado->result_array() as $Fila)
			{
				$Pedidos['reproceso'][$Fila['id_pedido']] = $Fila;
			}
			...
			if($Eres_Grupo)
			{
				$Consulta = '
					...
					from pedido_pedido
					where id_ped_secundario in ('.implode(',', $Id_Pedidos).')
				';
				$Resultado = $this->db->query($Consulta);
				foreach($Resultado->result_array() as $Fila)
				{
					unset($Pedidos[$Fila['id_ped_secundario']]);
				}
			}
